    ...
    public function index(Request $request)
    {
        ...
        if ($request->nonCompletDoss) {
            ...
            return $query
                ...
                ->join(DB::raw('(SELECT idTypeDossier, COUNT(idTypePiece) piecesTotal FROM composit_dossiers GROUP BY
            idTypeDossier) cd'), 'dossiers.IDTYPEDOSSIER', '=', 'cd.idTypeDossier')
                ->havingRaw('p.piecesExist < cd.piecesTotal')
                ->with('beneficiaire')
                ->with('type')
                ->with('service')
                ->with('pieces.type')
